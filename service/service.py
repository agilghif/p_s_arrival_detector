...
def restart(input_data: json) -> str:
    ...
    redis_client.set(f"{station_code}~data_p", f"{0}~{utc.strftime(settings.DATETIME_FORMAT)}~0")
    redis_client.set(f"{station_code}~data_s", f"{0}~{utc.strftime(settings.DATETIME_FORMAT)}~0")
