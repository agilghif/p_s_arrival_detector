...
# Service
wave_arrival_detector = bentoml.Service(
    "wave_arrival_detector",
    runners=[p_detector_runner, s_detector_runner, mag_runner, dist_runner],
)
...
def restart(input_data: json) -> str:
    ...
    # Create initial state
    redis_client.set(
        f"{station_code}~data_p", f"{0}~{utc.strftime(settings.DATETIME_FORMAT)}~0"
    )
    redis_client.set(
        f"{station_code}~data_s", f"{0}~{utc.strftime(settings.DATETIME_FORMAT)}~0"
    )
    redis_client.set(f"{station_code}~timer_s", utc.strftime(settings.DATETIME_FORMAT))

    return "OK"


# Wave detection service -----------------------------------------------------------------------------------------------
@wave_arrival_detector.api(
    input=JSON(pydantic_model=InputDataInference),
    output=JSON(pydantic_model=OutputDataInference),
)
def predict(input_data: json) -> json:
    # Unpack json
    x: np.ndarray = np.array(input_data.x)
    begin_time: datetime = datetime.strptime(
        input_data.begin_time, settings.DATETIME_FORMAT
    )
    station_code: str = input_data.station_code
    ...
    if station_code in pipelines:
        ...
        pipeline = Pipeline(
            settings.P_MODEL_PATH, settings.WINDOW_SIZE, name=station_code
        )
    ...
    # Extract insight
    p_arrival_detected, p_arrival_time, p_arr_id, new_p_event = examine_prediction(
        prediction_p, station_code, begin_time, is_p=True
    )
    ...
    timer_s: datetime = datetime.strptime(
        redis_client.get(f"{station_code}~timer_s").decode("UTF8"),
    ...
    if p_arrival_detected and new_p_event:
        ...
        redis_client.set(
            f"{station_code}~timer_s", p_arrival_time.strftime(settings.DATETIME_FORMAT)
        )
    ...
    if timer_s - begin_time <= settings.S_WAVE_DETECTION_DURATION:
        ...
        # Extract insight
        s_arrival_detected, s_arrival_time, s_arr_id, new_s_event = examine_prediction(
            prediction_s, station_code, begin_time, is_p=False
        )
...
@wave_arrival_detector.api(
    input=JSON(pydantic_model=InputDataMagLoc),
    output=JSON(pydantic_model=OutputDataMagLoc),
)
...
@wave_arrival_detector.api(
    input=JSON(pydantic_model=InputDataMagLogRecalc),
...
def recalculate(input_data: json) -> json:
    ...
    station_latitudes: np.ndarray = np.array(input_data.station_latitudes).astype(
        np.complex128
    )
    station_longitudes: np.ndarray = np.array(input_data.station_longitudes).astype(
        np.complex128
    )
...
def examine_prediction(
    ...
    arrival_detected, arrival_pick_idx, arrival_count = pick_arrival(
        prediction, threshold=settings.P_THRESHOLD
    )

    # Present result
    # -- Convert p_arrival_idx to timestamps
    arrival_time = begin_time + timedelta(
        seconds=arrival_pick_idx / settings.SAMPLING_RATE
    )

    # Check last earthquake occurrence, note that le = last earthquake
    le_id, le_time, le_count = (
        redis_client.get(f"{station_code}~data_{'p' if is_p else 's'}")
        .decode("UTF-8")
        .split("~")
    )
    le_id: int = int(le_id)
    ...
    if arrival_detected:
        ...
        # Save state
        redis_client.set(
            f"{station_code}~data_{'p' if is_p else 's'}",
            f"{le_id}~{str(arrival_time)}~{arrival_count}",
        )
...
def pick_arrival(
    prediction: np.ndarray, threshold=0.5, window_size=settings.WINDOW_SIZE
